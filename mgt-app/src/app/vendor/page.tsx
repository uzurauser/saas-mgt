...
export default async function VendorsPage() {
  ...
  try {
    summaryA = await prisma.summaryVendorService.findMany({ select: { clientId: true, vendorId: true, vendorServiceId: true } })
    summaryB = await prisma.summaryVendorServiceCspService.findMany({ select: { clientId: true, vendorId: true, vendorServiceId: true } })
  ...
  // vendorIdごとに集約
  const vendorIdMap: Record<number, { clientIds: Set<number>, vendorServiceIds: Set<number> }> = {}
  for (const t of tuples) {
    if (!vendorIdMap[t.vendorId]) {
      vendorIdMap[t.vendorId] = { clientIds: new Set(), vendorServiceIds: new Set() }
    }
  ...
  const vendorRows = vendors.map(vendor => {
    ...
    const clientNames = group ? Array.from(group.clientIds).map(cid => clients.find(c => c.id === cid)?.name || "").filter(Boolean) : []
    const serviceNames = group ? Array.from(group.vendorServiceIds).map(vid => vendorServices.find(s => s.id === vid)?.name || "").filter(Boolean) : []
    return {
      vendorId: vendor.id,
        ...
        <Table className="text-gray-900">
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="w-20 text-center">Vendor ID</TableHead>
              <TableHead className="w-48 truncate">Vendor Name</TableHead>
              <TableHead className="w-64 truncate">Vendor Services</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {vendorRows.map((row) => (
              <TableRow key={row.vendorId} className="hover:bg-blue-50 transition">
                <TableCell className="w-56 truncate">{row.clients}</TableCell>
                <TableCell className="w-20 text-center">{row.vendorId}</TableCell>
                <TableCell className="w-48 truncate">{row.vendorName}</TableCell>
