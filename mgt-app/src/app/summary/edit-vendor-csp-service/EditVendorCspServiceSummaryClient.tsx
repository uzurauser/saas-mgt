...
export default function EditVendorCspServiceSummaryClient({
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    // 必須項目がすべて埋まっている行のみ送信
    const validRows = rows.filter(row => row.client && row.vendor && row.service && row.csp && row.cspService)
  ...
  return (
    <main className="p-0 flex justify-center items-start min-h-screen bg-[#f8fafc]">
      <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-100 w-full max-w-full mt-8">
        <h1 className="text-xl font-bold mb-4">Edit Vendor Service CSP Summary</h1>
        ...
        <table className="w-full border mb-4 text-xs">
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr>
                <td colSpan={13} className="text-center py-4 text-gray-400">No data</td>
              </tr>
            ) : (
              rows.map((row, idx) => (
                <tr key={row.id ?? `new-${idx}`}>
                  ...
                  <td className="border px-2 py-1">
                    <select className="border rounded px-2 py-1 w-full min-w-[120px]" value={row.client} onChange={e => handleChange(idx, "client", e.target.value)}>
                  ...
                  <td className="border px-2 py-1 min-w-[160px]">
                    <CreatableSelect
                      isClearable
                      options={vendorOptions.map(opt => ({ value: opt.name, label: opt.name }))}
                      value={row.vendor ? { value: row.vendor, label: row.vendor } : null}
                      onChange={opt => handleChange(idx, "vendor", opt ? opt.value : "")}
                      onCreateOption={inputValue => handleChange(idx, "vendor", inputValue)}
                      placeholder="Vendor"
                      styles={{ container: base => ({ ...base, minWidth: 0 }), control: base => ({ ...base, minHeight: 32, fontSize: '0.875rem' }) }}
                    />
                  </td>
                  <td className="border px-2 py-1 min-w-[160px]">
                    <CreatableSelect
                      ...
                      value={row.service ? { value: row.service, label: row.service } : null}
                      onChange={opt => handleChange(idx, "service", opt ? opt.value : "")}
                      onCreateOption={inputValue => handleChange(idx, "service", inputValue)}
                  ...
                  {/* CSP Columns */}
                  <td className="border px-2 py-1">
                    <select className="border rounded px-2 py-1 w-full min-w-[120px]" value={row.csp} onChange={e => handleChange(idx, "csp", e.target.value)}>
                      <option value="">Select</option>
                      {cspOptions.map(opt => (
                        <option key={opt.id} value={opt.name}>{opt.name}</option>
                      ))}
                  ...
                  <td className="border px-2 py-1">
                    <select className="border rounded px-2 py-1 w-full min-w-[120px]" value={row.cspService} onChange={e => handleChange(idx, "cspService", e.target.value)}>
                      <option value="">Select</option>
                  ...
                  <td className="border px-2 py-1 text-center">
                    <button type="button" className="border border-gray-300 bg-white text-gray-800 hover:bg-gray-50 text-xs px-2 py-1 rounded" onClick={() => handleDelete(idx)}>Delete</button>
                  </td>
                </tr>
              ))
            )}
        ...
        <div className="flex justify-between items-center mt-2">
          <button type="button" className="bg-slate-700 hover:bg-slate-800 text-white text-sm px-4 py-2 rounded" onClick={handleAdd}>Add</button>
          <div className="flex items-center gap-2">
            <button type="button" className="px-4 py-2 rounded border border-gray-300 bg-white text-gray-800 hover:bg-gray-50 text-sm" onClick={handleCancel} disabled={isPending}>Cancel</button>
            <button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white text-sm px-4 py-2 rounded" disabled={isPending}>Save</button>
          </div>
        </div>
        </form>
      </div>
