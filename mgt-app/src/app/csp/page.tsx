...
export default async function CspPage() {
  ...
  try {
    summaryA = await prisma.summaryVendorServiceCspService.findMany({ select: { clientId: true, cspId: true, cspServiceId: true } })
    summaryB = await prisma.summaryOutsourcingServiceCspService.findMany({ select: { clientId: true, cspId: true, cspServiceId: true } })
  ...
  const cspIdMap: Record<number, { clientIds: Set<number>, cspServiceIds: Set<number> }> = {}
  ...
  const cspRows = csps.map(csp => {
    ...
    const serviceNames = group ? Array.from(group.cspServiceIds).map(sid => cspServices.find(s => s.id === sid)?.name || "").filter(Boolean) : []
    return {
      cspId: csp.id,
      cspName: csp.name,
  ...
  return (
    <main className="flex-1 p-8 bg-[#f8fafc] min-h-screen">
      <section className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">CSP List</h1>
        ...
        <Table className="text-gray-900">
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="w-20 text-center">CSP ID</TableHead>
              <TableHead className="w-48 truncate">CSP Name</TableHead>
              <TableHead className="w-64 truncate">CSP Services</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {cspRows.map((row) => (
